...
public class MarathonRestTest extends JerseyTest {
  ...
  @Test
  public void updateMarathonWithRawJSON() {
    // Arrange
    Long expectedId = 1000L;
    String expectedName = "Updated Marathon";
    String expectedCity = "a different city";
    String updatedMarathonJSON = "{ \"id\":\"1000\",\"name\":\"Updated Marathon\",\"city\":\"a different city\"}";

    // Act
    final Response response = target("marathon")
        .path("{id}").resolveTemplate("id", expectedId.toString())
        .request(MediaType.APPLICATION_JSON_TYPE)
        .put(Entity.json(updatedMarathonJSON));
    // Ici c'est jackson qui fait le unmarshalling du JSON en objet Java
    Marathon actualMarathon = response.readEntity(Marathon.class);
    String actualName = actualMarathon.getName();
    Long actualId = actualMarathon.getId();
    String actualCity = actualMarathon.getCity();
  ...
  @Test
  public void updateMarathonWithObject() {
    // Arrange
    Long expectedId = 1000L;
    String expectedName = "Updated Marathon";
    String expectedCity = "a different city";
    Marathon expectedMarathon = new Marathon(expectedId, expectedName, expectedCity);

    // Act
    final Response response = target("marathon")
        .path("{id}").resolveTemplate("id", expectedId.toString())
        .request(MediaType.APPLICATION_JSON_TYPE)
        .put(Entity.json(expectedMarathon));
    Marathon actualMarathon = response.readEntity(Marathon.class);
    String actualName = actualMarathon.getName();
    Long actualId = actualMarathon.getId();
    String actualCity = actualMarathon.getCity();
